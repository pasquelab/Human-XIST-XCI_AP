...
- Group 1: 
   - CUT&RUN data of TSC and EXMC from this paper:
...
- Group 3:
   - CUT&RUN data of H9-NK2 TSC and CUT&Tag data of CT27 TSC:
...
Each group of data was analysed separately in different folder. Fastq files were placed in a `fastq/raw/` subfolder on a cluster and processed using the provided snakemake workflows and simple bash script, in sequential order. The Snakemake workflows are composed of 4 files:
...
In addition to these 4 files, the snakemake workflows sometimes require other input files (R scripts, genome size files, etc.). In case of the XenoF.R script, line 3 has to be modified prior to execution of the worflow to adapt the library paths for R packages to your cluster repository structure.
...
The output files `.regions.bed.gz`, `.idxstats.txt` , `macs2_enlarged.bed` were then processed on R to generate Figure 2B-C-D-E-F (see link to quarto). All other input files required for plotting are given in `R/input-files/`.  
